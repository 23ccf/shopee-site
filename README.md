...
| **数据新鲜度** | 卡片左上角标出「这条数据是什么时候抓的」；超 7 天转红，提醒该重录 |
| **显示字段** | 自己勾选卡片显示哪些信息。关掉不关心的字段卡片会变矮，一屏能看更多商品 |
| **关注词看板** | 常搜的词存成一排标签，点一下即筛选，并显示当前命中数与**日环比涨跌**（词的市场在涨还是跌） |
| **收藏** | 收藏后可分组、写备注，一键导出选品清单 CSV（含链接 / 分组 / 备注 / 收藏时间） |
...
| `shopee_gate_v1` | 月销门槛（默认 30） |
| `shopee_cardfields_v1` | 卡片显示字段配置 |
| `shopee_fav_v2` | 收藏：`{ id: { g: 分组, n: 备注, t: 收藏时间 } }` |
| `shopee_fav_groups_v1` | 自定义收藏分组名 |
| `shopee_kw_v1` | 关注关键词 + 每日命中件数（保留 30 天） |

> 旧键 `shopee_fav`（纯 id 数组）在首次打开时会被**读取并迁移**到 `shopee_fav_v2`，
> 但它本身**不会被改写或删除** —— 万一新版有问题，回滚旧代码仍能读到完整收藏。

都在你本机浏览器里，清缓存会丢；重要数据请用页面上的「导出 CSV」留档。
收藏页另有<b>「导出选品清单」</b>，可直接导出成带链接与备注的 CSV。
