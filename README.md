...
| **搜索** | 商品名 / 店铺 模糊匹配，**支持简繁互通**（搜简体「运动鞋」能命中繁体「運動鞋」，繁简混写也行） |
| **筛选** | 价格区间、月销门槛（「最低评分」在全库无评分数据时自动隐藏 —— 否则一设就清空列表还不解释） |
| **排序** | 月销、周销、总销、价格（价格缺失的商品一律排最后，不会显示成 NT$0） |
| **详情弹窗** | 大图 + 完整字段 + 跳转原商品页 |
| **删除** | 勾选后删除，写入 `deleted.json`，扩展同步时不会再录回来 |
| **数据新鲜度** | 卡片左上角标出「这条数据是什么时候抓的」；超 7 天转红，并在卡片底部给出 **「↻ 已 N 天，去重录」** 直达原商品页（打开即被录制器被动刷新，零风控） |
| **数据体检条** | 商品库顶部一行汇总**整库字段缺口**：缺价格 / 缺名称·主图 / 价格待校验 / 总销已修正 / 超 7 天未更新，并指引到卡片上的「↻ 去重录」逐件补齐，附 **「只看待补数据 N 件 ›」** 一键切到缺口视图。可一键关闭并记住 |
| **数据缺口 → 动作** | 「↻ 去重录」不只在数据陈旧时给：**缺价格**给「↻ 缺价格，去重录」，**缺名称/主图**给「↻ 缺名称和图，去重录」。把「数据不可信」变成「点一下就能修」 |
...
> 这些板块原先读 `data/analysis.json`，而那是一个 2026-08-08 的空骨架、且没有程序生成它，
> 所以永远空白。现在改为**直接用当前在录商品客户端现算**，数据一到就立刻有内容。

| 板块 | 算法 | 依赖字段 |
|---|---|---|
| **🔥 热销** | 累计总销量 TOP50 | `sold_total` |
| **🚀 飙升** | 本机快照（`shopee_track_v1`）最近两次记录的总销量差 | 本地快照；无快照时给出引导而非空白 |
| **💰 价格带** | 按 NT\$0-100 / 100-300 / 300-500 / 500-1000 / 1000+ 分档，统计每档商品数与平均月销 | `price` / `month_sold`（价格缺失样本不计入） |
| **💎 蓝海词** | 对你的每个「关注词」算供需比：`log(1+均月销) / (log(1+均月销) + log(1+商品数))`。需求高、竞品少 → 指数高 | 关注词 + `month_sold` |
| **🔍 查商品** | 粘贴链接 / 「店铺ID 商品ID」→ **先查本地全量库**：录过就直接给出该商品卡片 + 最后采集时间（含被月销门槛隐藏的，并说明为何没进列表）；没录过则给出直达原商品页的链接去录制。**静态部署下不依赖后端**（此前无条件请求 `/api/product`，线上必然 404 —— 已修） | — |

**已被摘除的板块**（数据永远为空，留着只会误导）：
- 行业大盘 —— 完全按 `cats`（类目）分组，而录制器从未采集类目
- 榜单 —— 4 个分榜里 3 个依赖 `rating` / `reviews` / `liked` / `listed_at`，全未采集；唯一能用的「热销榜」与新的「🔥 热销」重复
- 类目销量分布 —— 同样依赖 `cats`

同理摘除的还有：排序里的「评分 / 评价 / 点赞 / 新品」四项（点了没任何变化），
以及只有「全部」一个选项的「品类 / 地区」下拉（选项为空时自动隐藏）。
...
### 📐 数据质量口径（2026-09-10 数据体检后统一）

线上 491 件的字段覆盖率实测：`shopid/itemid/sold_total/first_seen/last_seen` 100% ·
`month_sold` 95% · `name/img` 90% · `price` 85% · `week_sold` 80% ·
**`price_max` 0%（价格区间需录制器 v3.1.9 重录后才会产生）** · `rating/reviews/cats/loc` **0%**。

对应的显示规则：

| 情况 | 处理 |
|---|---|
| 价格缺失（74 件） | 显示「**未采集**」，不伪装成 `NT$0`；排序时排最后；评分里**不参与价格带项、分母同步扣掉该权重**（否则白扣 10 分把 74 件系统性压低） |
| 累计总销 < 月销（7 件） | 物理上不可能（月销是总销近 30 天的子集）→ 抬到月销并标「**✓ 总销已修正**」，原值留 `sold_total_raw`。不修的话热销榜按总销排序，真爆款会被排到最后 |
| 名称 / 主图缺失（51 件） | 卡片显示「（未采集到名称）」+ 占位图，并给「↻ 缺名称和图，去重录」 |
| `rating` / `reviews` 全为 0 | 卡片不显示「★ 0」；排行榜**整列不显示**；口碑权重自动并入销量并写明；「最低评分」筛选隐藏 |
| `cats` / `loc` 全为空 | 品类/地区下拉自动隐藏，不摆只有「全部」的死控件 |
| 数据超 7 天 | 角标转红 + 「↻ 已 N 天，去重录」 |
| 想集中补齐缺口 | 体检条上的 **「只看待补数据 N 件 ›」**：一键切到缺口视图，**按月销降序**—— 先补最值钱的；按钮变「✓ 正在只看待补数据 · 点此取消」，点一下恢复全部商品。关掉体检条会**同时退出该视图**，避免取消入口随体检条消失、把人困在几十件里 |

### 🧭 选品洞察（`insight.html`）

| 功能 | 说明 |
|---|---|
| **选品打分** | 月销 × 增长 × 价格带 × 竞争度（同款数）综合打分，S/A/B/C 分档；权重可拖动，实时重算 |
| **跨店比价** | 同一商品在不同店铺的价格排序 |
| **价格趋势** | 结合销售追踪的快照，画价格历史折线 |

> 数据质量口径（2026-09-10 对齐）：
> - `rating` / `reviews` 在 491 件里覆盖率 **0%**（从未被采集）→ 「口碑权重」自动并入销量并在统计条写明；
>   排行榜里的「评分 / 评价」两列**有数据才显示**，不摆一列恒为「—」/0 的噪音。
> - 价格缺失（74/491 件）显式显示「**未采集**」，不伪装成 NT$0；且该商品**不参与价格带评分、分母同步扣掉该权重**
>   —— 否则「没采到」会被当成「价格很差」，白扣 10 分把 74 件商品在排行榜上系统性压低。
> - 「只看分档」下拉**标出每档真实件数**（如 `S 档（≥85） · 0 件`），避免点进去看到空列表以为页面坏了。
> - 「增长」列需要两天以上记录，只有 1 天时给出说明而不是一列「—」。
